...
public class MapComponent : MonoBehaviour {
	...
	[SerializeField, OnValueChanged("OnSettingsChanged")]
	...
	void Init() {
		...
		octaveOffsets = GetOctaveOffsets(mapSettings.octaves, mapSettings.offsetSeed, mapSettings.fixedOffsetX,
			mapSettings.fixedOffsetY);
	}
	...
	public float GetMeshHeight(Vector3 targetPosition) {
		float height = mapSettings.meshHeightMultiplier *
		               mapSettings.meshHeightCurve.Evaluate(Perlin(targetPosition.x, targetPosition.z));
		return height;
	}
	...
	private void Tesselate() {
		...
		int[] triangles =
			new int[mapSettings.tesselation * mapSettings.tesselation * 2 *
			        3]; // 2 - polygon per quad, 3 - corners per polygon
